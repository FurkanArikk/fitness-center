...
start_db() {
    ...
    if docker ps -a --format '{{.Names}}' | grep -q "fitness-facility-db"; then
        echo -e "${YELLOW}Container exists but is not running. Starting existing container...${NC}"
        docker start fitness-facility-db
    else
    ...
    if ! docker ps --format '{{.Names}}' | grep -q "fitness-facility-db"; then
        echo -e "${RED}Error: Container failed to start. Checking logs...${NC}"
        docker logs fitness-facility-db
    ...
    for i in {1..5}; do
        ...
        if docker exec fitness-facility-db pg_isready -U fitness_user -d fitness_facility_db; then
            ...
            echo "  DB_HOST=localhost DB_PORT=5435 ./scripts/db-connect.sh"
            return 0
        fi
...
debug_db() {
    ...
    docker inspect --format='{{.State.Status}}' fitness-facility-db
    
    # Check if container is running
    if ! docker ps --format '{{.Names}}' | grep -q "fitness-facility-db"; then
        echo -e "${RED}Container exists but is not running.${NC}"
        echo -e "\n${YELLOW}Container Exit Code:${NC}"
        docker inspect --format='{{.State.ExitCode}}' fitness-facility-db
        
        echo -e "\n${YELLOW}Container Error:${NC}"
        docker inspect --format='{{.State.Error}}' fitness-facility-db
        
        echo -e "\n${YELLOW}Last 50 lines of container logs:${NC}"
        docker logs --tail 50 fitness-facility-db
        ...
        docker exec fitness-facility-db bash -c "cat /var/log/postgresql/postgresql*.log 2>/dev/null || echo 'No PostgreSQL logs found'"
    fi
}
...
reset_db() {
    ...
    if [[ $confirm == [yY] || $confirm == [yY][eE][sS] ]]; then
        ...
        if docker ps -a | grep -q fitness-facility-db; then
            docker rm -f fitness-facility-db
        fi
...
psql_db() {
    ...
    docker-compose exec postgres psql -U fitness_user -d fitness_facility_db
}
