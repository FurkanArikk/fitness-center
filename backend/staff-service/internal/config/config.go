...
func Load() (*Config, error) {
	...
	config := &Config{
		Server: ServerConfig{
			...
			ReadTimeout:  getEnvAsDuration("STAFF_SERVICE_READ_TIMEOUT", 15*time.Second),
			WriteTimeout: getEnvAsDuration("STAFF_SERVICE_WRITE_TIMEOUT", 15*time.Second),
			IdleTimeout:  getEnvAsDuration("STAFF_SERVICE_IDLE_TIMEOUT", 60*time.Second),
		},
		Database: DatabaseConfig{
...
func findServiceEnvFile() string {
	...
	serviceEnvPath := "/home/furkan/work/fitness-center/backend/staff-service/.env"
...
func findRootEnvFile() string {
	...
	// Try project root (backend/staff-service/..)
	envPath := filepath.Join(dir, "..", "..", ".env")
	if _, err := os.Stat(envPath); err == nil {
		return envPath
	}
