...
# Script to manage the Docker-based PostgreSQL database for staff-service
...
start_db() {
    ...
    if docker ps -a --format '{{.Names}}' | grep -q "fitness-staff-db"; then
        echo -e "${YELLOW}Container exists but is not running. Starting existing container...${NC}"
        docker start fitness-staff-db
    else
    ...
    if ! docker ps --format '{{.Names}}' | grep -q "fitness-staff-db"; then
        echo -e "${RED}Error: Container failed to start. Checking logs...${NC}"
        docker logs fitness-staff-db
    ...
    for i in {1..5}; do
        ...
        if docker exec fitness-staff-db pg_isready -U fitness_user -d fitness_staff_db; then
            ...
            echo "  DB_HOST=localhost DB_PORT=5433 ./scripts/db-connect.sh"
            return 0
        fi
...
debug_db() {
    ...
    if ! docker ps --format '{{.Names}}' | grep -q "fitness-staff-db"; then
        ...
        docker exec fitness-staff-db bash -c "cat /var/log/postgresql/postgresql*.log 2>/dev/null || echo 'No PostgreSQL logs found'"
    fi
}
...
psql_db() {
    ...
    docker-compose exec postgres psql -U fitness_user -d fitness_staff_db
}
